...
from .models import Schedule, Destination, AIRecommendation
...
class AITravelConsultForm(forms.Form):
    destination = forms.CharField(label="어디로 가고 싶으신가요?", max_length=100)
    duration = forms.CharField(label="여행 기간은 어떻게 되시나요? (예: 당일치기, 1박 2일 등)", max_length=100)
    departure = forms.CharField(label="어디에서 출발하시나요?", max_length=100)
    companions = forms.CharField(label="누구와 함께 여행하시나요?", max_length=100)
    date = forms.CharField(label="여행 예정일은 언제인가요?", max_length=100)
    theme = forms.CharField(label="가장 중요하게 생각하는 테마나 활동은 무엇인가요?", max_length=200)
    style = forms.CharField(label="여행은 여유롭게? 아니면 다양한 곳을 둘러보기?", max_length=100)
...
class TravelSurveyForm(forms.Form):
    ...
    num_people = forms.IntegerField(min_value=1, max_value=20, initial=1, label="여행 인원수")
    budget = forms.DecimalField(min_value=0, max_digits=10, decimal_places=0, label="예산 (원)")
...
class ScheduleForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            'budget': forms.NumberInput(attrs={'placeholder': '예: 300000', 'step': '1000', 'min': 0}),
            ...
            'age_group': forms.Select(choices=[
                ('', '선택하세요'),
                ('10대', '10대'), ('20대', '20대'), ('30대', '30대'),
                ('40대', '40대'), ('50대', '50대'), ('60대 이상', '60대 이상')
            ]),
    ...
    def clean_budget(self):
        ...
        return round(budget, -3) if budget else 0

# 4. AI 추천 결과 저장용 폼
class AIRecommendationForm(forms.ModelForm):
    class Meta:
        model = AIRecommendation
        fields = ['title', 'prompt', 'result_text', 'result_json', 'recommendation_type']
        widgets = {
            'prompt': forms.Textarea(attrs={'rows': 4}),
            'result_text': forms.Textarea(attrs={'rows': 8}),
        }
